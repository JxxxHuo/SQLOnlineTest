...
function execute_sql(tid, btn) {
    ...
    var genTable = function (arr) {
        ...
        return '<table class="uk-table" style="color:#98AFC7"><thead><tr>'
                + $.map(ths, function (th) {
                    var n = th.indexOf('!');
                    if (n > 1) {
                        th = th.substring(n + 1);
...
function _mdSetCodeResult(btn, result, isHtml, isError) {
    ...
    if ($r.get(0) === undefined) {
        $(btn).after('<div class="x-code-result x-code uk-alert" style="color:red"></div>');
...
function initMarkdownRun() {
    $('pre>code').each(function (i, code) {
        var
                ...
                nohightlight = (_.find(classes, function (s) {
                    return s.indexOf('language-nohightlight') >= 0;
                }) || '').trim(),
                x_run = (_.find(classes, function (s) {
                    return s.indexOf('language-x-') >= 0;
                }) || '').trim();
...
function initMarkdownInfo() {
    $('pre>code').each(function () {
        var
                $code = $(this),
                classes = ($code.attr('class') || '').split(' '),
                warn = (_.find(classes, function (s) {
                    return s.indexOf('language-!') >= 0;
                }) || '').trim(),
                info = (_.find(classes, function (s) {
                    return s.indexOf('language-?') >= 0;
                }) || '').trim();
        if (warn || info) {
...
function execute_sql2(tid, btn) {
    var code = _mdGetCode(tid);
    var genTable = function (arr) {
        if (arr.length === 0) {
            return 'Empty result set';
        }
        var columns=arr[0].columns;
        var values=arr[0].values;
